...
public class MemberDao {
	...
	public boolean join(HttpServletRequest request) throws AlertException {
		...
		String sql = "INSERT INTO member (memId, memPw, memNm, socialId) VALUES(?,?,?,?)";
	...
	public void checkJoinData(HttpServletRequest request) throws AlertException {
		 ...
		 * 3. 아이디 중복 여부 체크(O)
		 * 4. 비밀번호 확인시 일치 여부
		 * 5. 소셜 가입회원인 경우 비밀번호 항목 유효성 검사 패스
		 */
		...
		/** 비밀번호 자리수 체크 S */
		String memPw = null;
		if (socialMember == null) { // 이 오류 찾는데 한시간 걸렸습니다 ㅠㅠ
		...
		/** 비밀번호 확인시 일치 여부 S */
		if (socialMember == null) { // 이 오류도 찾는데 한시간 ㅠㅠ
