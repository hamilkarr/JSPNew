...
public class FileUpload {
	...
	/**
	 * 파일 업로드 처리 - 파일 데이터 -> 파일 DB에 기록한 후 증감번호인 idx -> 서버 저장용 파일명 - 일반데이터 ->
	 * HashMap형태로 따로 저장 -> 메서드 체이닝 방식으로 get 함수 호출시
	 * 
	 * @param request
	 */
		...
		try {
			...
			for (FileItem item : items) {
				if (!item.isFormField()) {
					...
					/**
					 * 1. 파일 정보를 DB에 기록 2. 추가된 증감번호 -> idx 3. 서버에 업로드될 경로 생성 증감번호 -> 10진수 (나머지 연산자
					 * -> 균등하게 10개 폴더에 저장) 0번, 1번, 2번, 3번, 4번 ... 9번 4. 파일 업로드 FileItem.write(File
					 * 인스턴스) - 실제 서버에 올라갈 파일 경로 uploadPath + "/" + (idx % 10) + "/" + idx;
					 */

					// DB에 파일 정보 기록
					long gid = System.currentTimeMillis();
					if (params.get("gid") != null) {
						Long.valueOf(params.get("gid"));
					}
					String sql = "Insert Into fileInfo (gid, originalName, mimeType) Value (?,?,?)";
	...
	public void download(HttpServletResponse response, int idx) throws IOException {
		...
		/**
		 * FileInputStream + BufferedInputStream - 읽고 SerlvetOutputStream -
		 * getOutputStream -> 화면에 출력
		 */
		...
		response.setHeader("Content-Type", "application/octet-strea");
	...
	public boolean delete(int idx) {
		/**
		 * 1. 파일 정보를 idx로 조회 2. 파일 삭제 3. 파일 정보 DB 삭제 
		 */
