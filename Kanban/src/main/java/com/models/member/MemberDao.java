...
public class MemberDao {
	...
	private MemberDao() {
	}; // 기본 생성자 private -> 외부 생성 X, 내부에서만 생성 O
	...
	public void checkJoinData(HttpServletRequest request) throws Exception {
		/**
		 * 1. 필수 항목 체크(O) 2. 아이디 체크 - 1) 자리수 체크(8~30) - O - 2) 알파벳 + 숫자만 입력(O) - 3) 아이디
		 * 중복 체크 (O) 3. 비밀번호 - 1) 자리수 체크(8자리 이상~) (O) - 2) 복잡성 체크 (보류) - 비밀번호에는 숫자, 알파벳,
		 * 특수문자가 각각 1개씩 포함 - 3) 비밀번호 확인 4. 휴대전화번호(필수 항목 X) (O) - 휴대전화번호가 들어오면 - 휴대전화번호
		 * 형식에 맞는지 체크
		 */
		/** 필수 항목 체크 S */
		String[] required = null;
		if (socialMember == null) {// 일반회원
			required = new String[] { "memId//아이디를 입력해 주세요.", "memPw//비밀번호를 입력해 주세요.", "memPwRe//비밀번호를 확인해 주세요.",
					"memPwHint//비밀번호 힌트를 입력해 주세요.", "memNm//회원명을 입력해 주세요." };
		} else { // 소셜 회원
			required = new String[] { "memId//아이디를 입력해 주세요.", "memNm//회원명을 입력해 주세요." };
		}
	...
	public void checkUpdateData(HttpServletRequest request) throws Exception {
		if (request == null)
			return;
		/**
		 * 1. 필수 항목 체크 (회원명) - O 2. 휴대전화번호 -> 변경이 있는 경우 형식 체크(O) 3. 비밀번호 변경 시도 하는 경우 ->
		 * 비밀번호 복잡성, 정확성 체크(O)
		 */
		String[] required = { "memNm//회원명을 입력하세요." };
	...
	public void checkCellPhone(String cellPhone) throws Exception {
		/**
		 * 1. 통일성 있도록 숫자로만 추출(숫자가 아닌 문자만 제거 -> 숫자) 2. 패턴 체크
		 */
		cellPhone = cellPhone.replaceAll("[^0-9]", "");
	...
	public boolean login(HttpServletRequest request, String memId, String memPw) throws Exception {
		/**
		 * 1. memId를 통해 회원 정보를 조회 2. 회원 정보가 조회가 되면(실제 회원 있으면) - 비밀번호를 체크 3. 비밀번호도 일치? ->
		 * 세션 처리(회원 번호 - memNo 세션에 저장)
		 */
